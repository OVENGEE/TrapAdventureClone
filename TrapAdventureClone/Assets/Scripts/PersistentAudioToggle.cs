...
public class PersistentAudioToggle : MonoBehaviour
{
    ...
    // Slightly smaller volume button.
    private const float ButtonSize = 68f;

    private const float TopRightMarginX = 250f;

    // Slightly lower than before.
    private const float TopRightMarginY = 46f;

    private const float DefaultFeedbackVolume = 0.7f;
    ...
    private static void CreatePersistentToggle()
    {
        ...
        GameObject toggleObject =
            new GameObject("Persistent Audio Toggle");

        instance =
            toggleObject.AddComponent<PersistentAudioToggle>();
    ...
    private void BuildToggle()
    {
        volumeSprite =
            Resources.Load<Sprite>(
                VolumeButtonResourcePath
            );

        muteSprite =
            Resources.Load<Sprite>(
                MuteButtonResourcePath
            );
    ...
    {
        GameObject buttonObject =
            new GameObject("Audio Mute Button");
    ...
    private void ApplyMuteState()
    {
        ...
        BackgroundMusicPlayer.SetMuted(
            isMuted
        );

        AudioFeedback.MasterVolume =
            isMuted
                ? 0f
                : DefaultFeedbackVolume;
    ...
    private static void EnsurePersistentEventSystem()
    {
        ...
        if (persistentEventSystem == null)
        {
            ...
            if (
                persistentEventSystem.transform.parent
                != null
            )
            {
            ...
            DontDestroyOnLoad(
                persistentEventSystem.gameObject
            );
        }
